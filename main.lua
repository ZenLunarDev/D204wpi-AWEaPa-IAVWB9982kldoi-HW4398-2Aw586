...
if not WindUI then
    error("[xEz Hub] Cannot load WindUI")
end
...
local TabMain = Window:Tab({ Title = "หลัก", Icon = "home" })
local TabSettings = Window:Tab({ Title = "ตั้งค่า", Icon = "settings" })
local TabStats = Window:Tab({ Title = "สถิติ", Icon = "bar-chart-2" })
local TabCredits = Window:Tab({ Title = "เครดิต", Icon = "star" })
...
BlackText.Text = "xEz Hub - หน้าจอดำ"
...
task.spawn(function()
    ...
    while BlackFrame.Parent do
        ...
        if BlackFrame.Visible then
            pcall(function()
                BlackText.Text = "xEz Hub - หน้าจอดำ" .. dots[i]
            end)
            i = i % #dots + 1
        end
    end
...
local function toggleAntiAFKKey(enabled)
    if Connections.AntiAFKKeyLoop then
        Connections.AntiAFKKeyLoop:Disconnect()
...
local function toggleAntiKick(enabled)
    if Connections.AntiKickLoop then
        Connections.AntiKickLoop:Disconnect()
    ...
    while true do
        ...
        if State.AutoSkipWave then
            local skipRemote = Workspace:FindFirstChild("SkipWave")
                or game:GetService("ReplicatedStorage"):FindFirstChild("SkipWave")
...
StatsLabels.Uptime = TabStats:Paragraph({ Title = "เวลาเปิด" .. ": 00:00:00", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Wins = TabStats:Paragraph({ Title = "ชนะ" .. ": 0", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Fails = TabStats:Paragraph({ Title = "พลาด" .. ": 0", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Rate = TabStats:Paragraph({ Title = "อัตราสำเร็จ" .. ": 0%", Desc = "", Image = "rbxassetid://0" })
StatsLabels.BestTime = TabStats:Paragraph({ Title = "เร็วสุด" .. ": 0.0s", Desc = "", Image = "rbxassetid://0" })
StatsLabels.AvgTime = TabStats:Paragraph({ Title = "เฉลี่ย" .. ": 0.0s", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Ping = TabStats:Paragraph({ Title = "ปิง" .. ": 0ms", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Speed = TabStats:Paragraph({ Title = "ความเร็ว" .. ": 900", Desc = "", Image = "rbxassetid://0" })
StatsLabels.Respawns = TabStats:Paragraph({ Title = "เกิดใหม่" .. ": 0", Desc = "", Image = "rbxassetid://0" })
...
print("[xEz Hub] v" .. SCRIPT_VERSION .. " loaded. WindUI Edition.")
